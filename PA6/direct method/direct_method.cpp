...
void DirectPoseEstimationSingleLayer(
    ...
    cv::Mat img1_show, img2_show, img3_show;
    cv::cvtColor(img1, img1_show, cv::COLOR_GRAY2BGR);
    cv::cvtColor(img2, img2_show, cv::COLOR_GRAY2BGR);
    cv::cvtColor(img2, img3_show, cv::COLOR_GRAY2BGR);        
    for (auto &px: px_ref) {
    ...
    for (size_t i = 0; i < goodProjection.size(); i++) {
    	auto pr = goodRef[i];
    	auto pc = goodProjection[i];
        if (pc[0] > 0 && pc[1] > 0) {
            cv::circle(img3_show, cv::Point2f(pc[0], pc[1]), 2, cv::Scalar(0, 250, 0), 2);
            cv::line(img3_show, cv::Point2f(pr[0], pr[1]), cv::Point2f(pc[0], pc[1]), cv::Scalar(0, 250, 0));
        }
    }
    
    cv::imshow("reference", img1_show);
    cv::imshow("current", img2_show);
    cv::imshow("point_flow", img3_show);
    cv::waitKey();
}
