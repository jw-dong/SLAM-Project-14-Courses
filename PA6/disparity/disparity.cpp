...
int main(int argc, char **argv) {
    ...
    Mat img_left = imread(file_left, 0);
    Mat img_right = imread(file_right, 0);
    Mat img_disparity = imread(file_disparity, 0);

    // key points, using GFTT here.
    vector<KeyPoint> kp1;
    Ptr<GFTTDetector> detector = GFTTDetector::create(500, 0.01, 20); // maximum 500 keypoints
    detector->detect(img_left, kp1);

    // now lets track these key points in the second image
    // first use single level LK in the validation picture
    vector<KeyPoint> kp2_single;
    vector<bool> success_single;
    OpticalFlowSingleLevel(img_left, img_right, kp1, kp2_single, success_single, true);

    // then test multi-level LK
    vector<KeyPoint> kp2_multi;
    vector<bool> success_multi;
    OpticalFlowMultiLevel(img_left, img_right, kp1, kp2_multi, success_multi);
    ...
    vector<uchar> status;
    vector<float> error;
    cv::calcOpticalFlowPyrLK(img_left, img_right, pt1, pt2, status, error, cv::Size(8, 8));

    // plot the differences of those functions
    Mat img2_single;
    cv::cvtColor(img_right, img2_single, COLOR_GRAY2BGR);
    ...
    Mat img2_multi;
    cv::cvtColor(img_right, img2_multi, COLOR_GRAY2BGR);
    ...
    Mat img2_CV;
    cv::cvtColor(img_right, img2_CV, COLOR_GRAY2BGR);
    ...
    for (int i = 0; i < kp2_multi.size(); i++) {
        if (success_multi[i]) {
            float disparity_ref = img_disparity.at<uchar>(kp1[i].pt.y, kp1[i].pt.x);
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    if (inverse == false) {
                        // Forward Jacobian
						J[0] = -1.0 * (GetPixelValue(img2, kp.pt.x + dx + x + 1, kp.pt.y + dy + y) 
								- GetPixelValue(img2, kp.pt.x + dx + x - 1, kp.pt.y + dy + y)) / 2;
						J[1] = -1.0 * (GetPixelValue(img2, kp.pt.x + dx + x, kp.pt.y + dy + y + 1)
								- GetPixelValue(img2, kp.pt.x + dx + x, kp.pt.y + dy + y - 1)) / 2;
                    } else {
                        // Inverse Jacobian
                        // NOTE this J does not change when dx, dy is updated, so we can store it and only compute error
                        J[0] = -1.0 * (GetPixelValue(img1, kp.pt.x + x + 1, kp.pt.y + y)
                        		- GetPixelValue(img1, kp.pt.x + x - 1, kp.pt.y + y)) / 2;
                        J[1] = -1.0 * (GetPixelValue(img1, kp.pt.x + x, kp.pt.y + y + 1)
